...
async function processBlocksAndInserts(reader: JWWReader): Promise<void> {
  ...
  for (let i = 0; i < blocks.size(); i++) {
    ...
    console.log(`  Base point: (${block.baseX}, ${block.baseY}, ${block.baseZ})`);
  ...
  for (let i = 0; i < hatches.size(); i++) {
    ...
    for (let j = 0; j < hatch.loops.size(); j++) {
      ...
      for (let k = 0; k < loop.edges.size(); k++) {
        ...
        switch (edge.type) {
          case 1: edgeTypes.line++; break;
          case 2: edgeTypes.arc++; break;
          case 3: edgeTypes.ellipse++; break;
          case 4: edgeTypes.spline++; break;
        }
      }
...
async function processDimensions(reader: JWWReader): Promise<void> {
  ...
  const dimTypeNames = [
    'LINEAR', 'ALIGNED', 'RADIAL', 'DIAMETRIC', 
    'ANGULAR', 'ANGULAR3P', 'ORDINATE'
  ];
  ...
  for (let i = 0; i < dimensions.size(); i++) {
    ...
    switch (dim.type) {
      ...
      case 4: // ANGULAR
        ...
        console.log(`  Line 1: (${dim.dpx1}, ${dim.dpy1}) to (${dim.dpx2}, ${dim.dpy2})`);
        // Additional points would be in separate fields
        break;
        
      case 5: // ANGULAR3P
        console.log(`3-point angular dimension: ${dim.text}`);
        console.log(`  Points: (${dim.dpx1}, ${dim.dpy1}), (${dim.dpx2}, ${dim.dpy2})`);
        break;
...
async function processImages(reader: JWWReader): Promise<void> {
  ...
  for (let i = 0; i < images.size(); i++) {
    ...
    const uLength = Math.sqrt(image.ux * image.ux + image.uy * image.uy + image.uz * image.uz);
    const vLength = Math.sqrt(image.vx * image.vx + image.vy * image.vy + image.vz * image.vz);
    console.log(`  Display size: ${uLength.toFixed(2)} x ${vLength.toFixed(2)}`);
    ...
    if (image.clippingState && image.clippingVertices.size() > 0) {
      console.log(`  Clipping boundary: ${image.clippingVertices.size()} vertices`);
    }
  }
}
...
async function processLargeFile(file: File): Promise<void> {
  ...
  const progressCallback = Module.addFunction((current: number, total: number) => {
...
interface ProcessedData {
  ...
  inserts: Array<{ blockName: string; x: number; y: number; scale: number; angle: number }>;
  ...
  images: Array<{ file: string; x: number; y: number; width: number; height: number }>;
}
